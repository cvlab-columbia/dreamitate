...
class SetupCallback(Callback):
    ...
    def on_exception(self, trainer: pl.Trainer, pl_module, exception):
        ...
        if not self.debug and trainer.global_rank == 0:
            print("Summoning checkpoint in on_exception().")
...
class ImageLogger(Callback):
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if (
            ...
            if 'video_dict' in images:
                video_dict = images.pop('video_dict')
                ...
                for k, v in video_dict.items():
                    filename = "{}_gs-{:06}_e-{:06}_b-{:06}.mp4".format(
                        k, pl_module.global_step, pl_module.current_epoch, batch_idx
                    )
                    path = os.path.join(root, filename)
                    cv = convert_video(v)

                    # mediapy.write_video(path, convert_video(v))
                    my_save_video(path, cv, fps=7)
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        if self.log_train and (
            not self.disabled and (pl_module.global_step > 0 or self.log_first_step)
        ):
    ...
    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx):
        if self.log_train and (
            self.log_before_first_step and pl_module.global_step == 0
        ):
...
def my_save_video(dst_fp, frames, fps):
...
if __name__ == "__main__":
    ...
    try:
        ...
        def melk(*args, **kwargs):
            ...
            if trainer.global_rank == 0:
                print("Summoning checkpoint in melk().")
